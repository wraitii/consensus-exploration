...
import { ProposeMessage, VoteMessage, TimeoutMessage } from "./messages";
...
class Replica {
  ...
  tick() {
    ...
    if (this.lastVoteTime >= this.timeoutDelay) {
      const message = new TimeoutMessage(
        this.peer,
        this.lastVoteLevel,
        undefined
      );
      if (this.broadcastTimeout)
        ...
        this.peer.orchestrator.addSignal(
          new Signal(
            this.peer,
            this.proposerSelectorLogic.getProposer(this.lastVoteLevel + 2),
            message
          )
        );
  ...
  propose(signal: ProposeMessage) {
    ...
    // Safety checks
    if (!signal.commitCertificate) return; // TODO: handle genesis block?
    ...
    if (
      !signal.commitCertificate!.checkCertificate(
        this.proposerSelectorLogic.getAllPeers(signal.level - 1).length
      )
    ) {
    ...
    if (this.proposalsVoted.has(signal.level))
      // this doesn't actually check for "different", it should
    ...
    if (this.lastVoteLevel > signal.level)
      throw new Error(
        `Peer ${this.peer.name} already voted at a higher level ${this.lastVoteLevel}`
      );
    ...
    if (signal.level - 2 >= 0) {
      ...
      if (commitBlock) {
        ...
        console.warn(
          "Peer ",
          this.peer.name,
          "could not find block to commit at level",
          signal.level - 2
        );
    ...
    if (true) {
      const voteSignal = new Signal(
        this.peer,
        this.proposerSelectorLogic.getProposer(signal.level + 1),
        vote
      );
  ...
  vote(signal: VoteMessage) {
    ...
    if (
      !this.proposerSelectorLogic.isProposer(
        signal.propose.level + 1,
        this.peer
      )
    )
      return;

    this.votesReceived.set(
      signal.propose.level,
      this.votesReceived.get(signal.propose.level) || new Set()
    );
    this.votesReceived.get(signal.propose.level)!.add(signal.sender);

    const totalPeers = this.proposerSelectorLogic.getAllPeers(
      signal.propose.level
    ).length;
    if (
      this.votesReceived.get(signal.propose.level)!.size >=
      Math.floor((2 * totalPeers) / 3) + 1
    ) {
  ...
  proposeNewBlock(level: number, value: string, cheat?: Data<any>) {
    ...
    if (!buildOnTopOf) buildOnTopOf = cheat;
    ...
    if (
      (this.votesReceived.get(level - 1)?.size ?? 0) >
      (this.timeoutsReceived.get(level - 2)?.size ?? 0)
    ) {
      console.info(
        "Peer ",
        this.peer.name,
      ...
      message.withCommit(
        new FakeCommitCertificate(
          level - 1,
          buildOnTopOf as Block,
          this.votesReceived.get(level - 1)!
        )
      );
    } else {
      console.info(
      ...
      message.withCommit(
        new FakeCommitCertificate(
          level - 1,
          buildOnTopOf as Block,
          this.timeoutsReceived.get(level - 2)!
        )
      );
    }
  ...
  onTimeout(signal: TimeoutMessage) {
    ...
    this.timeoutsReceived.set(
      signal.level,
      this.timeoutsReceived.get(signal.level) || new Set()
    );
    this.timeoutsReceived.get(signal.level)!.add(signal.sender);

    if (this.byzantine) return;

    if (this.lastVoteLevel - 1 > signal.level) return; // we've already voted at a higher level, so ignore this past timeout info.
    ...
    const totalPeersAtTimeoutLevel = this.proposerSelectorLogic.getAllPeers(
      signal.level + 1
    ).length;
    if (
      this.lastVoteLevel < signal.level + 1 &&
      this.timeoutsReceived.get(signal.level)!.size >=
        Math.floor(totalPeersAtTimeoutLevel / 3) + 1
    )
    ...
    if (
      ...
      if (!this.hasProposedBlock.get(signal.level + 2)) {
        ...
        const nilBlock = new Block(
          "",
          (this.proposalsVoted.get(signal.level)?.data as Block) || null
        );
        const randomLetter = String.fromCharCode(
          65 + Math.floor(Math.random() * 26)
        );
